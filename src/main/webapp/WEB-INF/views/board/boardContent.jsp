...
<div class="container">
  ...
  <table class="table table-borderless m-0 p-0">
    <tr>
      <td class="text-left">
        <c:if test="${param.flag != 'search'}"><input type="button" value="돌아가기" onclick="location.href='boardList?pag=${param.pag}&pageSize=${param.pageSize}';" class="btn btn-warning"/> &nbsp;</c:if>
        <c:if test="${param.flag == 'search'}"><input type="button" value="돌아가기" onclick="location.href='boardSearch?pag=${param.pag}&pageSize=${param.pageSize}&search=${param.search}&searchString=${param.searchString}';" class="btn btn-warning"/> &nbsp;</c:if>
      </td>
      <td class="text-right">
...
<div class="container">
  ...
  <table class="table table-hover">
    <tr>
      ...
      <th>댓글일자</th>
      <th>접속IP</th>
      <th class="text-right">답글</th>
    </tr>
    <c:forEach var="replyVO" items="${replyVOS}" varStatus="st">
      <tr>
        <td>
        	<c:if test="${replyVO.re_step >= 1}">
	          <c:forEach var="i" begin="1" end="${replyVO.re_step}">&nbsp;&nbsp;</c:forEach> └▶ <!-- 화살표 옆에 띄우면 안되나?.. -->
          </c:if>
	        ${replyVO.nickName}
      	...
      	<td colspan="5" class="m-0 p-0">
	      	<div id="replyDemo${replyVO.idx}" style="display:none">
		      	<table class="table table-center">
		      		<tr>
		      			<td style="width:85%" class="text-left"> 답글 내용 :
		      				<textarea rows="4" name="contentRe" id="contentRe${replyVO.idx}" class="form-control">@${replyVO.nickName}</textarea> <!-- 이해안됨.. -->
		      			</td>
		      			<td style="width:15%">
		      				<br/>
		      				<p>작성자 : ${sNickName}</p>
		      				<p><input type="button" value="답글달기" onclick="replyCheckRe(${replyVO.idx},${replyVO.re_step},${replyVO.re_order})" class="btn btn-secondary btn-sm" /></p>
		      			</td>
		      		</tr>
		      	</table>
	      	</div>
