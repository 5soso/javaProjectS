...
<div class="container">
  ...
  <pre>
  	- 스프링 시큐리티(Spring Security) 프레임워크에서 제공하는 클래스중 하나로 비밀번호를 암호화 하는데 사용한다.
  		이것은 주로 자바 서버프로그램 개발을 위해 필요한 '인증 / 인가(권한부여)' 및 '보안기능'을 제공해주는 프레임워크에 속한다.
  		단방향 암호화 기법으로 복호화 되지 않는다.
  	- bCryptPasswordEncoder는 BCryt해싱함수를 사용하여 비밀번호를 인코딩해주는 메서드와 사용자에 의해 제출된 비밀번호를
  		저장소에 저장된 비밀번호와의 일치여부를 통해서 확인해주는 메소드로 제공된다.
  	- bCryptPasswordEncoder는 PasswordEncoder 인터페이스를 구현한 클래스이다.
  	- 사용되는 메소드? 
  	1) encode(java.lan.CharSequence) : 패스워드를 암호화 해주는 메소드이다.  반환타입은 String 이다.
  		 encode() 메서드는 sha-1에 의한 8Byte(64Bit)로 결합된 해시키를 랜덤하게 생성된 솔트(salt)를 지원한다.
  	2) machers(java.lang.CharSequence) : 제출된 인코딩 되지 않은 패스워드의 일치여부를 판단하기위해 인코딩된 패스워드와 비교/판단한다.
  		 반환타입은 boolean이다.
  </pre>
  <br/>
  <div>
